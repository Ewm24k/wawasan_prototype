...
(function () {
  ...
  function initCarousel(opts) {
    ...
    if (dotsWrap) {
      for (var i = 0; i < count; i++) {
        ...
        dot.type = 'button';
        dot.setAttribute('aria-label', (opts.dotLabel || 'Slaid') + ' ' + (i + 1));
  ...
  initCarousel({
    ...
    renderSlide: function (slide) {
      ...
      li.style.backgroundImage =
        'linear-gradient(180deg, rgba(7,27,61,0.15) 0%, rgba(7,27,61,0.65) 100%), url("' + slide.image + '")';
  ...
  if (form) {
    form.addEventListener('submit', function (e) {
      ...
      if (isModal) {
        // Give the visitor a moment to read the success message,
        // then hand control back to the landing page popup.
        setTimeout(notifyParentClose, 1600);
      }
    });
  }
})();
